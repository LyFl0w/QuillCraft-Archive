...
public class Account {
    ...
    public Account(final int id, final UUID uuid, final int quillCoins, final byte rankID, final Visibility visibility, final Map<Particles, Boolean> particles, final String languageISO) {
        this(id, uuid, null, quillCoins, rankID, visibility, particles, languageISO);
    }

    public Account(final int id, final UUID uuid, final UUID partyUUID, final int quillCoins, final byte rankID, final Visibility visibility, final Map<Particles, Boolean> particles, final String languageISO) {
        this.id = id;
        this.uuid = uuid;
    ...
    public void setParticles(Map<Particles, Boolean> particles) {
        this.particles = particles;
    ...
    public enum Visibility {
        ...
        public static Visibility getVisibilityByData(Material material) {
            final Optional<Visibility> optionalVisibility = Arrays.stream(values()).filter(visibility -> visibility.getMaterial() == material).findFirst();

            if (optionalVisibility.isPresent()) return optionalVisibility.get();

            throw new IllegalCallerException("Every player have a visibility");
        }
