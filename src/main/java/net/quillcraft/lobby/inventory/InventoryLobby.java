...
public class InventoryLobby {
    ...
    public static void setDefaultInventory(final Player player, final LanguageManager languageManager, final Account.Visibility visibility){
        ...
        playerInventory.setItem(2, new ItemBuilder(Material.PLAYER_HEAD, (byte)3).setName(languageManager.getMessage(Text.ACCOUNT_ITEM)).
                setSkullOwner(player).toItemStack());
        playerInventory.setItem(3, new ItemBuilder(Material.PUFFERFISH, (byte)3).setName(languageManager.getMessage(Text.FRIENDS_ITEM)).toItemStack());
