package net.quillcraft.highblock.database.request.island;
...
import net.quillcraft.highblock.utils.LocationUtils;
...
public class IslandRequest extends DefaultRequest {
    ...
    public boolean hasIsland(UUID uuid) throws SQLException {
        ...
        try (final PreparedStatement preparedStatement = connection.prepareStatement("""
            ...
            result = (0 != preparedStatement.executeQuery().getInt(1));
        }
    ...
    public int createIsland(UUID uuid, IslandDifficulty islandDifficulty, String worldPath, double x, double y, double z, float yaw, float pitch) throws SQLException {
        ...
        try (final PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO Island (id_difficulty) VALUES (?)")) {
            preparedStatement.setInt(1, islandDifficulty.getDifficulty());
            preparedStatement.executeUpdate();

            primaryKey = preparedStatement.getGeneratedKeys().getInt(1);
        }

        try (final PreparedStatement preparedStatement = connection.prepareStatement("UPDATE Island SET spawn_location = ? WHERE id = ?")) {
            preparedStatement.setString(1, LocationUtils.getStringFromPosition(worldPath + (worldPath.endsWith("/") ? "" : "/") + primaryKey, x, y, z, yaw, pitch));
